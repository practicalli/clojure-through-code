...
(def i-have-a-name-long
  (fn []
    "I am not a number, I am a named function
         - actually we call the name a symbol and it can be used as a reference to the function."))

(i-have-a-name-long)

;; You can shorten this process by using defn
(defn i-have-a-name-consise
  []
  "Oh, I am a new function definition, but have the same name (symbol).")

(i-have-a-name-consise)
...
;; using a local variables... (its not actually a variable as it doesn't vary)
...
(def johnny {:firstname "Johnny" :lastname "Rocket"})
...
(def people
  {:001 {:name "johnny Rocket"   :twitter "jr0cket"}
...
(def people-locations
  {:Birmingham [{:name "johnny Rocket"   :twitter "jr0cket"}
...
;; How does multi-arity work?
;; The parameters are bound to there arguments when compiled.
...
(comment
  (str "The location of London is " (-> location :london :gps :co-ords)))
...
;; defs are globally accessible slots to bind symbols to values
...
(defn optional-args-polly
  ([a]
   (optional-args-polly a nil))
  ([a b]
   (println "Required argument a is" a)
   (println "Optional argument b is" b)))

(optional-args-polly "first")
...
(process-contact "johnny" "Clojureville" "johnny@clj.com")
...
;; (process-contact "johnny" "Clojureville" "johnny@clj.com" "07911000123")
...
(process-contact-map {:name "johnny" :address "Clojureville" :email "johnny@clj.com"})
...
(process-contact-map {:name "johnny" :address "Clojureville" :email "johnny@clj.com" :phone "07911000123"})
...
(process-contact-vector-destructured ["johnny" "Clojureville" "johnny@clj.com"])

(process-contact-vector-destructured ["johnny" "Clojureville" "johnny@clj.com" "07911000123"])
...
(process-contact-map-destructured {:name "johnny" :address "Clojureville" :email "johnny@clj.com"})
...
(process-contact-map-destructured {:name "johnny" :address "Clojureville" :email "johnny@clj.com" :phone "07911000123"})
...
(defn display-name
  ...
  (let [{:keys [first-name last-name]} user]
    (when (not-any? nil? [first-name last-name])
      (str "My name is: " first-name " " last-name
           ", who can be reached at " (get user :email-address)))))

;; If both the first & last names are in the map passed as an argument then a combined
;; first & last name is returned
(display-name {:first-name    "johnny"
               :last-name     "Rocket"
               :email-address "johnny@jr0cket.co.uk"})

;; "My name is: johnny Rocket, who can be reached at johnny@jr0cket.co.uk"
